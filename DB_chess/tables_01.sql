...
CREATE TABLE table_country
	(
	country VARCHAR(4) PRIMARY KEY,-- alpha-[2,3,4] ISO 3166
	full_country_name VARCHAR(80) NOT NULL
	);
*/
...
CREATE TABLE chess_player
	...
	chess_title VARCHAR(8) REFERENCES chess_title(title) ON DELETE CASCADE ON UPDATE CASCADE NOT NULL, 
	first_name VARCHAR(80) NOT NULL,
	second_name VARChAR(80) NOT NULL,
	country VARCHAR(4) REFERENCES table_country(country), -- alpha-[2,3,4] ISO 3166 ot FK?
	birthday DATE,
...
CREATE TABLE tournament
	...
	category VARCHAR(80) REFERENCES tournament_category(category_name) ON DELETE CASCADE ON UPDATE CASCADE NOT NULL, 
	total_rounds SMALLINT, -- ~(2^(total_round+1) - 1) games
	date_start date NOT NULL,
	date_end date NOT NULL,
	country VARCHAR(4) REFERENCES table_country(country) ON DELETE CASCADE ON UPDATE CASCADE NOT NULL,
	city VARCHAR(80) NOT NULL,
	organizator_id INTEGER REFERENCES organizator(organizator_id) ON DELETE CASCADE ON UPDATE CASCADE,
	winner_id INTEGER REFERENCES chess_player(player_id)  ON DELETE CASCADE ON UPDATE CASCADE 
	);
...
CREATE TABLE player_participation
	...
	tournament_id INTEGER REFERENCES tournament(tournament_id) ON DELETE CASCADE ON UPDATE CASCADE NOT NULL,
	player_id INTEGER REFERENCES chess_player(player_id)  ON DELETE CASCADE ON UPDATE CASCADE NOT NULL,
...
CREATE TABLE game
	(
	participating_id_1 BIGINT REFERENCES player_participation(player_participation_id) ON DELETE CASCADE ON UPDATE CASCADE,
