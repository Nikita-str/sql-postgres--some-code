...
class Player:
    ...
    def get_strable_arr_v2(self):
        rat_ch = '{"rating": ['
        for i in range(0, self.tours):
            last = '},'
            if(i == self.tours - 1): last = '}'
            rat_ch += '{"date":"'+ get_date_str(self.rating_dates[i])+ '","rating":' + str(self.rating[i]) + last
        rat_ch += ']};'
        return rat_ch + gen_about()+ ';' + self.info

names = open("names.txt").read().split()
surnames = open("surnames.txt").read().split()
b_dates = [[12, 12, 1980], [11, 11, 1973], [5, 6, 1982], [1, 2, 1980] ]
players = [];
players_amount = 1_000_001  # TODO
...
#  GAME_STATS:
valid_point = ['0', '1', '-1'];
game_amount = 100_000_001#TODO:100_000_001
...
for i in range(1, game_amount):
    ...
    for k in range(0, gs):
        last = '},'
        if(k == gs - 1): last = '}'
        f.write('{"game_result":'+ch(valid_point)+',"game_time":"'+ch(valid_time_n)+'","game_viewers":'+ch(valid_view_n)+last)
    f.write(']};');
    '''
    #'''
    if(gs == 1):
